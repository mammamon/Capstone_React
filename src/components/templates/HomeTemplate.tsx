...
export const HomeTemplate = () => {
  ...
  const { cinemaList, isFetchingCinemaList } = useSelector(
  ...
  useEffect(() => {
    if (cinemaSchedule?.length > 0) {
      const selectedCinemaSchedule = cinemaSchedule.find(
        (schedule) => schedule.maHeThongRap === selectedCinemaList
      );
      if (
        selectedCinemaSchedule &&
        selectedCinemaSchedule.lstCumRap.length > 0
      ) {
  ...
  return (
    <div>
      ...
      <div className="cinema-zone flex">
        <div className="cinema-list flex-2 grid-cols-2 gap-4 mt-8 w-2/12">
          {cinemaList?.map((cinema) => (
            <Card
              ...
              hoverable
              className={
                selectedCinemaList === cinema.maHeThongRap ? "selected" : ""
              }
            >
              <img
                src={cinema.logo}
                ...
                onClick={() => {
                  ...
                  const selectedCinemaSchedule = cinemaSchedule.find(
                    (schedule) => schedule.maHeThongRap === cinema.maHeThongRap
                  );
                  if (
                    selectedCinemaSchedule &&
                    selectedCinemaSchedule.lstCumRap.length > 0
                    ...
                    setSelectedCumRap(
                      selectedCinemaSchedule.lstCumRap[0].maCumRap
                    );
                  }
                }}
              />
        ...
        <div
          className="cinema-location mt-8 w-4/12"
              ...
              {schedule.lstCumRap.map((cumRap) => (
                <div
                  ...
                  className={`cursor-pointer ${
                    cumRap.maCumRap === selectedCumRap ? "selected" : ""
                  }`}
                  onClick={() => {
        ...
        <div
          className="cinema-schedule mt-8 w-6/12"
          style={{ height: "500px", overflow: "auto" }}
        >
          {selectedCinemaList &&
            selectedCumRap &&
                        ...
                        <div className="showtimes">
                          {phim.lstLichChieuTheoPhim &&
                            [...phim.lstLichChieuTheoPhim]
                              .sort(
                                (a, b) =>
                                  new Date(a.ngayChieuGioChieu).getTime() -
                                  new Date(b.ngayChieuGioChieu).getTime()
                              )
                              .map((lichChieu) => (
